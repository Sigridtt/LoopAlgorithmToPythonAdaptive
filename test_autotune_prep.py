...
#Same test files as in oref0 tests/command-behaviour.tests.sh except the following 
# - battery
# - eddison battery
# - last reservoir
# - medtronic frequency
...
def generate_test_files():
    ...
    with open('pumpprofile.json', 'w') as f:
        json.dump(pumpprofile, f, indent=4)

    # Make dummy basal profile
    basal_profile = [
            {
                "i": 0,
                "start": "00:00:00",
    ...
    with open('basal_profile.json', 'w') as f:
        json.dump(basal_profile, f, indent=4)
    
    # Make fake autosens.json
    autosens ={
        "ratio": 0.8
    }
    with open('autosens.json', 'w') as f:
        json.dump(autosens, f, indent=4)
    '''
    # Make fake clock-zoned.json 
    clock_zoned = {
        "2018-09-05T09:44:11-05:00"
    }
    with open('clock_zoned.json', 'w') as f:
        json.dump(clock_zoned, f, indent=4)
    
    # Make a fake uploader.json to test the commands that extract values from it
    uploader ={
        50
    }
    with open('uploader.json', 'w') as f:
        json.dump(uploader, f, indent=4)
    '''
    ...
    with open('iob.json', 'w') as f:
        json.dump(iob, f, indent=4)
    ...
    with open('suggested.json', 'w') as f:
    ...
    with open('enacted.json', 'w') as f:
        json.dump(enacted, f, indent=4)
    #skipped battery

    # Make a fake reservoir.json to test the commands that extract values from it
    reservoir = 51.05

    with open('reservoir.json', 'w') as f:
        json.dump(reservoir, f, indent=4)    
    ...
    cal = [ {"date":1535947102261,"scale":1,"intercept":34058.44519508067,"slope":955.9093730012227,"type":"LeastSquaresRegression"} ]
    ...
    with open('carbhistory.json', 'w') as f:
        json.dump(carbhistory, f, indent=4)
    ...
    with open('raw_glucose.json', 'w') as f:
        json.dump(raw_glucose, f, indent=4)
    ...
    with open('last_temp_basal.json', 'w') as f:
        json.dump(last_temp_basal, f, indent=4)
    ...
    with open('temp_basal.json', 'w') as f:
        json.dump(temp_basal, f, indent=4)
...
#skip mmtune
    ...
    with open('pumphistory_zoned.json', 'w') as f:
        json.dump(pumphistory_zoned, f, indent=4)
    ...
    with open('autotune_data.json', 'w') as f:
        json.dump(autotune_data, f, indent=4)

    #make autotune treatments
    autotune_treatments = [
        {
            "insulin": None,
            "carbs": None,
    ...
    with open('autotune_treatments.json', 'w') as f:
        json.dump(autotune_treatments, f, indent=4)

    #make autotune entries
    autotune_entries =  [
        {
            "glucose": 54,
    ...
    with open('autotune_entries.json', 'w') as f:
        json.dump(autotune_entries, f, indent=4)

    #make insulin sensitivities
    ...
    with open('insulin_sensitivities.json', 'w') as f:
    ...
    with open('temptargets.json', 'w') as f:
        json.dump(temptargets, f, indent=4)

    # Make meal
    meal = {"carbs":36,"nsCarbs":36,"bwCarbs":0,"journalCarbs":0,"mealCOB":0,"currentDeviation":-2.85,"maxDeviation":2.98,"minDeviation":-0.42,"slopeFromMaxDeviation":-1.15,"slopeFromMinDeviation":0,"allDeviations":[-3,0,1,2,1,3],"lastCarbTime":1536522546000,"bwFound":False}
    
    with open('meal.json', 'w') as f:
        json.dump(meal, f, indent=4)
    ...
    with open('carb_ratios.json', 'w') as f:
    ...
    with open('bg_targets.json', 'w') as f:
...
def test_autotune_prep():

    with open('autotune_entries.json', 'r') as f:
        autotune_entries = json.load(f)
    with open('pumpprofile.json', 'r') as f:
        pumpprofile = json.load(f)
    

    prep_cfg = AutotunePrepConfig(
        basal_rate=pumpprofile["basalprofile"],
        isf=pumpprofile["isfProfile"],
        carb_ratio=pumpprofile["carb_ratio"],
    )

    categorized_entries = _categorize_points(autotune_entries, prep_cfg)
    print("categorized entries: ", categorized_entries)
